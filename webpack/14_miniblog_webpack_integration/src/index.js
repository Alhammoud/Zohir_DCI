...
const loadCreateBlogPost = () => {
    const createblogposthtml = `
    <div class="new-blogpost">
    <input type="text" placeholder="Blog-Titel eingeben" id="title" />
    <textarea rows="10" id="content"></textarea>
    <button id="createPostButton" onclick="createPost()">Artikel erstellen</button>
    </div>`;

    mainObj.innerHTML = createblogposthtml;

    const createPostButtonObj = document.getElementById('createPostButton');
    createPostButtonObj.onclick = createPost;
...
Access to fetch at 'http://localhost:3000/login' from origin 'http://localhost:8080' has been blocked by CORS policy: Response to preflight request doesn't pass access control check: The value of the 'Access-Control-Allow-Origin' header in the response must not be the wildcard '*' when the request's credentials mode is 'include'.
index.js:147 
